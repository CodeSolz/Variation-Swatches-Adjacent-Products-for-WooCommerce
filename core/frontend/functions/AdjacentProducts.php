...
class AdjacentProducts {
	...
	public function single_page_adjacent( $user_input ) {
		...
		// $vProduct = wc_get_product( $current_product_id );
		// $attributes = $vProduct->get_attributes();
		// if( $attributes ){
		// foreach( $attributes as $attribute_slug => $value ){
		// echo $attribute_slug;
		// $color = $vProduct->get_attribute( $attribute_slug  );
		// }
		// }
		// pre_print(
		// $attributes
		...
		if ( has_filter( 'Wvsap_attributes_list' ) ) {

		} else {
			// $color = $vProduct->get_attribute( 'pa_color' );
		}
